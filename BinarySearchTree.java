...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        // throws exception if two nodes aren't related
        if (child.up != parent) { throw new IllegalArgumentException("Two nodes aren't related"); }
        if (root == parent) { // checks to see if parent is root
            if (child.isRightChild()) { // checks to see if child is right child
                Node<T> varB = child.down[0];
                Node<T> varC = parent;
                Node<T> varD = child;
                varC.down[1] = varB; // sets right child of old parent to old child's child
                varC.up = varD; // sets old parent's parent to old child
                varD.up = null; // sets old child's parent to null since it is now root
                varD.down[0] = varC; // sets old child's child to old parent
                if (varB != null) varB.up = varC; // checks to see if old child's child exists and
                                                  // sets it's parent to old parent if so
                root = varD; // sets old child to new root
            } else {
                Node<T> varB = child.down[1];
                Node<T> varC = parent;
                Node<T> varD = child;
                varC.down[0] = varB; // sets left child of old parent to old child's child
                varC.up = varD; // sets old parent's parent to old child
                varD.up = null; // sets old child's parent to null since it is now root
                varD.down[1] = varC; // sets old child's child to old parent
                if (varB != null) varB.up = varC; // checks to see if old child's child exists and
                                                  // sets it's parent to old parent if so
                root = varD; // sets old child to new root
            }
            ...
            if (child.isRightChild()) {
                ...
                Node<T> varC = parent;
                Node<T> varD = child;
                Node<T> varF = parent.up;
                varC.down[1] = varB; // sets right child of old parent to old child's child
                varC.up = varD; // sets old parent's parent to old child
                varD.up = varF; // sets old child's parents to old parent's parent
                varD.down[0] = varC; // sets old child's child to old parent
                if (varB != null) varB.up = varC; // checks to see if old child's child exists and
                                                  // sets it's parent to old parent if so
                varF.down[0] = varD; // sets old parent's parent to old child's parent
            } else {
                Node<T> varB = child.down[1];
                Node<T> varC = parent;
                Node<T> varD = child;
                ...
                if (varB != null) varB.up = varC; // checks to see if old child's child exists and
                                                  // sets it's parent to old parent if so
                varF.down[1] = varD; // sets old parent's parent to old child's parent
            }
        }
    }
   ...
   /**
    ...
    public static boolean test1() {
        // tests a left rotation at the root
        ...
        String expected = "level order: [ 8, 5, 2, 3 ]\nin order: [ 2, 3, 5, 8 ]";
    ...
    public static boolean test2() {
        // tests a right rotation not at the root
        BinarySearchTree testTree = new BinarySearchTree();
        testTree.insert(5);
        testTree.insert(2);
        testTree.insert(8);
        ...
        testTree.insert(10);
        testTree.insert(7);
        testTree.rotate(testTree.root.down[1].down[0], testTree.root.down[1]);
        String expected = "level order: [ 5, 2, 7, 1, 3, 8, 10 ]\n"
            + "in order: [ 1, 2, 3, 5, 7, 8, 10 ]";
        String actual = testTree.toString();
        if (!expected.equals(actual)) return false;
        return true;
    }
    ...
    public static boolean test3() {
        // tests a left rotation not at the root
        BinarySearchTree testTree = new BinarySearchTree();
        testTree.insert(5);
        testTree.insert(2);
        testTree.insert(8);
        ...
        testTree.insert(10);
        testTree.insert(7);
        testTree.rotate(testTree.root.down[0].down[1], testTree.root.down[0]);
        String expected = "level order: [ 5, 3, 8, 2, 7, 10, 1 ]\n"
            + "in order: [ 1, 2, 3, 5, 7, 8, 10 ]";
        String actual = testTree.toString();
        if (!expected.equals(actual)) return false;
        return true;
